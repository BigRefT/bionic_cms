...
module ActionView::Helpers
  module FormHelper
    def fckeditor(object_name, method, options = {})
      width = options[:width].nil? ? '100%' : options.delete(:width)
      height = options[:height].nil? ? '100%' : options.delete(:height)
      toolbarSet = options[:toolbarSet].nil? ? 'Default' : options.delete(:toolbarSet)
      id = options[:id] ||= name

      inputs = text_area(object_name, method, options)
      js_path = "#{controller.relative_url_root}/javascripts"
      base_path = "#{js_path}/fckeditor/"
  ...
  class FormBuilder
    class_eval do
      def fckeditor(method, options = {})
        options[:object] = @object
        @template.fckeditor(@object_name, method, options)
      end
    end
  end

  module FormTagHelper
    def fckeditor_tag(name, content = nil, options = {})
      width = options[:width].nil? ? '100%' : options.delete(:width)
      height = options[:height].nil? ? '100%' : options.delete(:height)
      toolbarSet = options[:toolbarSet].nil? ? 'Default' : options.delete(:toolbarSet)
      id = options[:id] ||= name

      inputs = text_area_tag(name, content, options)
      js_path = "#{controller.relative_url_root}/javascripts"
      base_path = "#{js_path}/fckeditor/"
      content_for :additional_script do
        javascript_tag("document.observe('dom:loaded',function(){\n" <<
