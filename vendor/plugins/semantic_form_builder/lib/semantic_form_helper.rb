module SemanticFormHelper
  ...
  def wrapping(type, field_name, label, field, options = {})
    ...
    to_return << %Q{<span class="input-prefix">#{options[:prefix]}#{"/" if options[:prefix].last != "/"}&nbsp;</span>} if options[:prefix]
    to_return << field
    to_return << %Q{<span class="input-suffix">&nbsp;#{options[:suffix]}&nbsp;</span>} if options[:suffix]
