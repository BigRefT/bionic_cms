class SemanticFormBuilder < ActionView::Helpers::FormBuilder
  ...
  def field_settings(method, options = {}, tag_value = nil)
    ...
    required = options[:required] ? options.delete(:required) : false
    options[:class] ||= ""
    options[:class] += required ? " required" : ""
    label += "<strong><sup>*</sup></strong>" if required
  ...
  def submit_and_cancel(submit_name, cancel_name, options = {})
    ...
    wrapping("submit", nil, "", submit_button+cancel_button.html_safe, options)
  end

  def submit_or_back(submit_name, back_path, options = {})
    submit_button = @template.submit_tag(submit_name, options)
    cancel_button = ' or ' + @template.link_to('Back', back_path)
    wrapping("submit", nil, "", submit_button+cancel_button.html_safe, options)
  end
  ...
  def radio_button_group(method, values, options = {})
    ...
    values.each do |value|
      ...
      radio_button = @template.radio_button(@object_name, method, tag_value, options.merge(:object => @object, :help => help, :required => nil))
  ...
  def check_box_group(method, values, options = {})
    ...
    values.each do |value|
      ...
      check_box = @template.check_box(@object_name, method, options.merge(:object => @object, :required => nil), checked_value, unchecked_value)
      selections << boolean_field_wrapper(
